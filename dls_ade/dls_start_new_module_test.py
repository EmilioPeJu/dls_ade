...
class MakeFilesPythonTest(unittest.TestCase):
    ...
    @patch('dls_ade.dls_start_new_module.os.mkdir')
    def test_opens_correct_file_and_writes_setup(self, mkdir_mock):
        ...
        mock_login = patch('dls_ade.dls_start_new_module.os.getlogin', return_value=getlogin)

        with patch.object(builtins, 'open', self.open_mock):
            with mock_login:
                dls_ade.dls_start_new_module.make_files_python(module)
...
class MakeFilesToolsTest(unittest.TestCase):
    ...
    def test_opens_correct_file_and_writes_build(self):
        ...
        with patch.object(builtins, 'open', self.open_mock):
            dls_ade.dls_start_new_module.make_files_tools(module)
...
class MakeFilesDefaultTest(unittest.TestCase):
    ...
    def test_opens_correct_file_and_writes_gitignore(self):
        ...
        with patch.object(builtins, 'open', self.open_mock):
            dls_ade.dls_start_new_module.make_files_default(module)

        file_handle_mock = self.open_mock()
        self.open_mock.assert_any_call(".gitignore", "w")

        file_handle_mock.write.assert_any_call(default_files['.gitignore'].format(**self.test_input))
