...
from dls_ade.bitbucketserver import BitbucketServer, json


class InitTest(unittest.TestCase):

    def test_init(self):
        server = BitbucketServer("test@url.ac.uk",
                                 "TestUser", "CrypticPassword123")

        self.assertEqual("test@url.ac.uk", server.url)
...
class GetServerRepoListTest(unittest.TestCase):
    ...
    def test_get_project_list(self, get_mock):
        ...
        get_mock.assert_called_once_with(
            'test@url.ac.uk/rest/api/1.0/projects',
    ...
    def test_get_server_repo_list(self, _, get_mock):
        ...
        self.assertIn(
            'test@url.ac.uk/rest/api/1.0/projects/S/repos', calls)
        self.assertIn(
            'test@url.ac.uk/rest/api/1.0/projects/I/repos', calls)
...
class CreateRemoteRepoTest(unittest.TestCase):
    ...
    def test_create_repo(self, post_mock):
        ...
        post_mock.assert_called_once_with(
            'test@url.ac.uk/rest/api/1.0/projects/S/repos',
            auth=('TestUser', 'CrypticPassword123'),
    ...
    def test_repo_exists_then_extract_message_for_error(self, post_mock):
