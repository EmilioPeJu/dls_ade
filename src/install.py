...
class dls_install(easy_install):
    """DLS specific version of easy_install, with versioned script names and
    comments in the wrapper scripts to point to the script function location"""
    
    def install_wrapper_scripts(self, dist):
        """Install the generated python scripts to their final location"""
        if not self.exclude_scripts:
            for args in get_script_args(dist):
                # add version to end of script name
                args = list(args)
                old_name = args[0]
                if args[0].endswith(".py"):
                    args[0] = "versioned/"+old_name[:-3] + "-" + dist.version + ".py"
                else:
                    args[0] = "versioned/"+old_name + "-" + dist.version
                import os
                os.system("ln -fs "+os.path.join(self.script_dir,args[0])+" "+os.path.join(self.script_dir,old_name))
                print "versioned script name", args[0]
                entry = str(dist.get_entry_info("console_scripts",old_name))
                args[1]+="\n# Entry point: "+entry
                args[1]+="\n# File location: "+os.path.join(dist.location,dist.project_name.replace(".","/"),entry.split(":")[-2].split(".")[-1].replace(".py","")+".py")
                self.write_script(*args)

# inherited code for reference:
...
# __all__ = [
#     'samefile', 'dls_install', 'PthDistributions', 'extract_wininst_cfg',
...
#         setup(
#             script_args = ['-q','dls_install', '-v']+argv,
